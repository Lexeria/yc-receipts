...
function App() {
  ...
  function onAdd(res: CreateResults, newItem: Receipt) {
    ...
    setListReceipts((prev) => {
      if (prev) {
        const res = { ...prev, receipts: [...prev.receipts, newItem] };
        return res;
      }
      return prev;
    });
  ...
  return (
    <div>
      ...
      <Flex bg={"yellow.300"} p={5} justifyContent={"space-between"}>
        <Heading>Рецепты онлайн</Heading>
        <Button
          leftIcon={<AddIcon />}
          colorScheme="orange"
          variant="solid"
      ...
      >
        {receipts?.receipts.map((item) => (
          <WrapItem key={item.receipt_id}>
            <Card maxW={"500px"}>
              <CardHeader>
                <Heading size={"md"}>{item.name}</Heading>
              </CardHeader>
              <CardBody whiteSpace={"pre-wrap"}>{item.receipt}</CardBody>
            </Card>
          </WrapItem>
        ))}
      </Wrap>
      <Flex bg={"yellow.100"} justifyContent={"space-between"} p={5}>
        <Text>Версия фронта {packageJson.version}</Text>
        <Text>Версия бекэнда {backendInfo.backend_version}</Text>
        <Text>Номер реплики {backendInfo.replica_id}</Text>
      </Flex>
    </div>
  );
}
